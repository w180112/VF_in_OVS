				   ...
				   const struct pci_device_id *ent)
{
	...
	cards_found++;
	netlink.adapter = adapter;
	netlink.hw = &adapter->hw;

	//printk("mem addr=%x, %x\n",netlink.adapter, netlink.hw);
	/*call netlink*/
...
static void nl_data_ready(struct sk_buff *skb)
{
    ...
    struct ixgbe_mbx_info *mbx = &netlink.hw->mbx;
    int i=0; /* for for loop index */
    ...
    //printk("mem addr=%x, %x, %x\n",netlink.adapter, netlink.hw, mbx);
	...
	nlh->nlmsg_len,
        ...
        nlh->nlmsg_pid,
        nlh->nlmsg_seq);*/
    netlink.msg.type = (*(netlink_msg_t*)NLMSG_DATA(nlh)).type;
    memcpy(netlink.msg.vf,(*(netlink_msg_t*)NLMSG_DATA(nlh)).vf,sizeof(netlink.msg.vf));
    netlink.msg.vlan = (*(netlink_msg_t*)NLMSG_DATA(nlh)).vlan;
    memcpy(netlink.msg.mac,(*(netlink_msg_t*)NLMSG_DATA(nlh)).mac,sizeof(netlink.msg.mac));
	//strtoul((char*)NLMSG_DATA(nlh), &p, 16);
    printk("netlink.msg.type = %d, vlan = %d\n",netlink.msg.type, netlink.msg.vlan);
	/*type == 0x0d, defined in ixgbe_mbx.h*/
    if (netlink.msg.type == IXGBE_VF_OF_VLAN) {
	u32 msg[3];
	msg[0] = netlink.msg.type;
	msg[2] = netlink.msg.vlan;
	for(i=0; netlink.msg.vf[i]!=0; i++) { 
	    msg[1] = netlink.msg.vf[i] - 1; //for first vf is 0
	    printk("vf = %d\n", msg[1]);
    	    spin_lock_bh(&netlink.adapter->mbx_lock);
	//netlink.retval = ixgbevf_write_mgs_read_ack(netlink.hw,netlink.msg,netlink.msg,256);
	    netlink.retval = mbx->ops.write_posted(netlink.hw, msg, 256, 0);	
	    spin_unlock_bh(&netlink.adapter->mbx_lock);
	}
    }
    else if (netlink.msg.type == IXGBE_VF_OF_MAC) {
	//
    }
    if (netlink.retval)
        printk("error\n");
